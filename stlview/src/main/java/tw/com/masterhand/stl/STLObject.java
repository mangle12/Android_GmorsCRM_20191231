...
public class STLObject {
    ...
    private int getIntWithLittleEndian(byte[] bytes, int offset) {
        return (0xff & stlBytes[offset]) | ((0xff & stlBytes[offset + 1]) << 8) | ((0xff &
                stlBytes[offset + 2]) << 16) | ((0xff & stlBytes[offset + 3]) << 24);
    }
        ...
        final AsyncTask<byte[], Integer, float[]> task = new AsyncTask<byte[], Integer, float[]>() {
                ...
                for (int i = 0; i < vertext_size; i++) {
                    for (int n = 0; n < 3; n++) {
                        normal_array[i * 9 + n * 3] = Float.intBitsToFloat(getIntWithLittleEndian
                                (stlBytes, 84 + i * 50));
                        normal_array[i * 9 + n * 3 + 1] = Float.intBitsToFloat
                                (getIntWithLittleEndian(stlBytes, 84 + i * 50 + 4));
                        normal_array[i * 9 + n * 3 + 2] = Float.intBitsToFloat
                                (getIntWithLittleEndian(stlBytes, 84 + i * 50 + 8));
                    }
                    float x = Float.intBitsToFloat(getIntWithLittleEndian(stlBytes, 84 + i * 50 +
                            12));
                    float y = Float.intBitsToFloat(getIntWithLittleEndian(stlBytes, 84 + i * 50 +
                            16));
                    float z = Float.intBitsToFloat(getIntWithLittleEndian(stlBytes, 84 + i * 50 +
                            20));
                    adjustMaxMin(x, y, z);
                    vertex_array[i * 9] = x;
            ...
            protected float[] doInBackground(byte[]... stlBytes) {
                ...
                try {
                    if (isText(stlBytes[0])) {
                        Log.i(getClass().getSimpleName(), "trying text...");
                        processResult = processText(new String(stlBytes[0]));
                    } else {
                        Log.i(getClass().getSimpleName(), "trying binary...");
                        processResult = processBinary(stlBytes[0]);
                    }
                } catch (Exception e) {
                }
                if (processResult != null && processResult.length > 0 && normal_array != null &&
                        normal_array.length > 0) {
            ...
            protected void onPostExecute(float[] vertexList) {
                ...
                if (normal_array.length < 1 || vertex_array.length < 1) {
                    Toast.makeText(context, context.getString(R.string.error_fetch_data), Toast
                            .LENGTH_LONG).show();
                    return;
                }
