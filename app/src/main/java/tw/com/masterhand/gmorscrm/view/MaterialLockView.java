...
public class MaterialLockView extends View {
    public static class Cell implements Parcelable {
        ...
        /**
         * Gets the ID.It is counted from left to right, top to bottom of the matrix, starting by
         * zero.
         *
         * @return the ID.
         */
     ...
     * mode is set. The entire
     * animation should take this constant * the length of the pattern to complete.
     */
    private static final int MILLIS_PER_CIRCLE_ANIMATING = 700;

    /**
     * This can be used to avoid updating the display for very small motions or noisy panels. It
     * didn't seem to have
     ...
     * cells of the complete
     * pattern unless we are animating, in which case we use this to hold the cells we are
     * drawing for the in progress
     * animation.
     */
    private boolean[][] mPatternDrawLookup = new boolean[LOCK_SIZE][LOCK_SIZE];

    /**
     * the in progress point: - during interaction: where the user's finger is - during
     * animation: the current tip of
     * the animating line
     */
    ...
    public MaterialLockView(Context context, AttributeSet attrs) {
        ...
        mBackgroundColor = ContextCompat.getColor(context, R.color.gray_dark);
        typedArray.recycle();

        mPathPaint.setColor(mSuccessColor);
    ...
    /**
     * Set whether the view is in stealth mode. If {@code true}, there will be no visible
     * feedback as the user enters
     * the pattern.
     *
     ...
     *
     * @return current displaying pattern. <b>Note:</b> This is an independent list with the
     * view's pattern itself.
     */
...
//                                mFastOutSlowInInterpolator, cellState, null);
    ...
    private void startSizeAnimation(float start, float end, long duration, Interpolator interpolator, final CellState state,
    ...
    private void drawCircle(Canvas canvas, float centerX, float centerY, float size, boolean partOfPattern, float alpha) {
        ...
        mBackgroundPaint.setAlpha((int) (alpha * 255));
        canvas.drawCircle(centerX, centerY, size / 2, mBackgroundPaint);
        ...
        canvas.drawCircle(centerX, centerY, mDotSize / 2, mDotPaint);
    }
